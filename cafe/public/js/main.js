...
$(function () {
    ...
    /**SEARCH */
        ...
        $("#search").on("keyup", function() {
            ...
            if (val) {
            val = val.toLowerCase();
            $.each(sphamArray, function(_,obj) {
            
                if (obj.name.toLowerCase().indexOf(val) != -1) {
                $(".spham").append('<div class="card m-2 text-center" style="width:20rem"><img src="'+obj.image+'" style="height:15em"/><div class="card-body"><div class="card-body"><div class="fs-3 fw-bold text-dark">'+obj.name+'</div><br><div class="card-title h6"> Giá: '+obj.gia+'.000đ</div><a href="shop.php"><button type="button" class="btn btn-outline-dark">Mua Sản Phẩm</button></a></div></div>');
                }
            });
            }
            $(".not-found").toggle($(".spham").find("img").length==0);
        });
    var sphamArray = [
        {
        name: "Cà Phê Đá",
